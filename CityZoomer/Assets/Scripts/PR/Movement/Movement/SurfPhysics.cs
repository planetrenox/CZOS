...
namespace Fragsurf.Movement
{
    public class SurfPhysics
    {
        ...
        /// </summary>
        public static int
            groundLayerMask = LayerMask.GetMask(new string[] {"Default", "Ground", "Player clip"}); //(1 << 0);
        ...
        public static float wallBounce = 2f;
        public static float rampBounce = 1.3f;
        public static Vector3 velocityBeforeImpact = new Vector3();
            ...
            ISurfControllable surfer = null)
        {
            ...
            for (int i = 0; i < numOverlaps; i++)
            {
                    ...
                    _colliders[i].transform.rotation, out direction, out distance))
                {
                    ...
                    if (rb != null && !rb.isKinematic) // Collision with rigidbody
                    {
                        rb.AddForceAtPosition(velocityProjected * velocityMultiplier * rigidbodyPushForce, origin,
                            ForceMode.Impulse);
                        velocity = velocityBeforeImpact;
                    }
                    else // Collision with ramp or wall
                    {
                        velocity = velocityBeforeImpact;
                    }
                }
            ...
            if (velocity.magnitude < 10) velocityBeforeImpact = Vector3.zero;
        }
            ...
            Vector3 direction, float distance, Vector3 forwardVelocity, ISurfControllable surfer)
        {
            ...
            if (wallTrace.hitCollider == null ||
                Vector3.Angle(Vector3.up, wallTrace.planeNormal) <= surfer.moveData.slopeLimit) return false;
            ...
            if (upTrace.hitCollider != null) upDistance = upTrace.distance;

            // Don't bother doing the rest if we can't move up at all anyway
            if (upDistance <= 0f) return false;
            ...
            if (forwardTrace.hitCollider != null) forwardDistance = forwardTrace.distance;
            ...
            if (downTrace.hitCollider != null) downDistance = downTrace.distance;
            ...
            if (stepAngle > surfer.moveData.slopeLimit) return false;
            ...
            float airCap, float deltaTime)
        {
            ...
            // Adjust pmove vel.
            for (int i = 0; i < 3; i++) result[i] += accelspeed * wishdir[i];
        ...
        public static int Reflect(ref Vector3 velocity, Collider collider, Vector3 origin, float deltaTime)
        {
            ...
            for (int bumpcount = 0; bumpcount < numBumps; bumpcount++)
            {
                ...
                if (trace.fraction == 1)
                {
                    //Debug.Log(new Vector2(primalVelocity.x, primalVelocity.z).magnitude);
                    // this was causing reflection to break/freeze at shallow angles
                ...
                if (numplanes == 1)
                {
                    for (int i = 0; i < numplanes; i++)
                    {
                        if (_planes[i][1] > SurfSlope)
                        {
                            // floor or slope
                            //return blocked;
                            ClipVelocity(originalVelocity, _planes[i], ref newVelocity, rampBounce);
                            originalVelocity = newVelocity;
                        }
                        else
                        {
                            ClipVelocity(originalVelocity, _planes[i], ref newVelocity, wallBounce);
                        }
                    }

                    velocityBeforeImpact = newVelocity;
                    originalVelocity = newVelocity;
                }
                else
                {
                    ...
                    {
                        ClipVelocity(originalVelocity, _planes[i], ref velocity, 1f);
                        ...
                        for (j = 0; j < numplanes; j++)
                        {
                            if (j != i)
                            {
                                ...
                                if (Vector3.Dot(velocity, _planes[j]) < 0) break;
                            }
                        }

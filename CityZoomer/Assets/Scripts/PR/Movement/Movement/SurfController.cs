...
namespace Fragsurf.Movement
{
    public class SurfController
    {
        ...
        public void ProcessMovement(ISurfControllable surfer, MovementConfig config, float deltaTime)
        {
            ...
            {
                if (_surfer.moveData.velocity.y <= 0f) jumping = false;

                // apply gravity
                if (_surfer.groundObject == null)
                {
                    _surfer.moveData.velocity.y -= (_surfer.moveData.gravityFactor * MovementConfig.gravity * _deltaTime);
            ...
            _surfer.moveData.velocity = Vector3.ClampMagnitude(_surfer.moveData.velocity, MovementConfig.maxVelocity);
            ...
            else
            {
                ...
                while (velocityDistLeft > 0f)
                {
                    ...
                    //Debug.Log(_surfer.moveData.rigidbodyPushForce);
                    //Debug.Log(amountThisLoop / initialVel);
        ...
        private int currentStep = 0; // 21 audio clips in total
                ...
                case MoveType.Walk:
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        if (madeStep == false) // if activated, it means player made a step. 
                        {
                            SurfCharacter.footstepsAudioSource.PlayOneShot(SurfCharacter.footstepAudioArrayStatic[currentStep]);
                            currentStep++;
                            if (currentStep == 21) currentStep = 0;
                            madeStep = true;
                        }
                        ...
                        {
                            if (!wasSliding) slideSpeedCurrent = Mathf.Clamp(slideSpeedCurrent * _config.slideSpeedMultiplier, _config.minimumSlideSpeed, _config.maximumSlideSpeed);
                        ...
                        {
                            if (slideDelay > 0f) slideDelay -= _deltaTime;

                            if (wasSliding) slideDelay = _config.slideDelay;
                        ...
                        if (crouching) speed = _config.crouchSpeed;
                        ...
                        float maxVelocityMagnitude = MovementConfig.maxVelocity;
        ...
        private void UnderwaterPhysics()
        {
            ...
            // Swimming upwards
            if (Input.GetButton("SourceJump")) _surfer.moveData.velocity.y += _config.swimUpSpeed * _deltaTime;
            ...
            if (waterJumpTrace.hitCollider != null && Vector3.Angle(Vector3.up, waterJumpTrace.planeNormal) >= _config.slopeLimit && Input.GetButton("SourceJump") && !_surfer.moveData
            .cameraUnderwater && 
            movingForwards)
                _surfer.moveData.velocity.y = Mathf.Max(_surfer.moveData.velocity.y, MovementConfig.jumpForce);
        }
        ...
        private void LadderCheck(Vector3 colliderScale, Vector3 direction)
        {
            ...
            RaycastHit[] hits = Physics.BoxCastAll(_surfer.moveData.origin, Vector3.Scale(_surfer.collider.bounds.size * 0.5f, colliderScale), Vector3.Scale(direction, new Vector3(1f, 0f, 1f)), Quaternion.identity,
            ...
            foreach (RaycastHit hit in hits)
            {
                ...
                if (ladder != null)
                {
                    ...
                    {
                        if (hit.normal.y < 0f) allowClimb = false;
                        else
                        {
                            if (ladderAngle <= _surfer.moveData.slopeLimit) allowClimb = false;
                        }
                    }
                    else if (hit.normal.y != 0f) allowClimb = false;
                    ...
                    if (allowClimb)
                    {
                        ...
                        if (_surfer.moveData.climbingLadder == false)
                        {
                            ...
                            else _surfer.moveData.ladderClimbDir = Vector3.up;
                        }
                    }
                }
            }
        ...
        private void ApplyFriction(float t, bool yAffected, bool grounded)
        {
            ...
            if (_speed > 0.0f) _newSpeed /= _speed;
        ...
        private Vector3 AirInputMovement()
        {
            ...
            return SurfPhysics.AirAccelerate(_surfer.moveData.velocity, wishDir, wishSpeed, MovementConfig.airAcceleration, _config.airCap, _deltaTime);
        }
        ...
        private void GetWishValues(out Vector3 wishVel, out Vector3 wishDir, out float wishSpeed)
        {
            ...
            for (int i = 0; i < 3; i++) wishVel[i] = forward[i] * _surfer.moveData.forwardMove + right[i] * _surfer.moveData.sideMove;
            wishVel[1] = 0;
        ...
        private void Jump()
        {
            if (!PR.UI.Toggle_AutoHop_READ) _surfer.moveData.wishJump = false;

            _surfer.moveData.velocity.y += MovementConfig.jumpForce;
            jumping = true;
        }
        ...
        private bool CheckGrounded()
        {
            ...
            if (trace.hitCollider == null || groundSteepness > _config.slopeLimit || (jumping && _surfer.moveData.velocity.y > 0f))
            {
                ...
                if (movingUp && _surfer.moveType != MoveType.Noclip) _surfer.moveData.surfaceFriction = _config.airFriction;
        ...
        private void SetGround(GameObject obj)
        {
            ...
            else _surfer.groundObject = null;
        }
        ...
        public void Crouch(ISurfControllable surfer, MovementConfig config, float deltaTime)
        {
            ...
            if (grounded) uncrouchDown = false;

            // Crouching input
            if (grounded) crouchLerp = Mathf.Lerp(crouchLerp, wantsToCrouch ? 1f : 0f, _deltaTime * _surfer.moveData.crouchingSpeed);
            else if (!grounded && !wantsToCrouch && crouchLerp < 0.95f) crouchLerp = 0f;
            else if (!grounded && wantsToCrouch) crouchLerp = 1f;
            ...
            if (crouchLerp > 0.9f && !crouching)
            {
                ...
                foreach (Transform child in playerTransform)
                {
                    ...
                    if (child.name != "Waypoint Arrow") child.localPosition = new Vector3(child.localPosition.x, child.localPosition.y * crouchingHeight, child.localPosition.z);
                }
            ...
            else if (crouching)
            {
                ...
                if (!canUncrouch) crouchLerp = 1f;
            }

            // Changing camera position
            if (!crouching)
                _surfer.moveData.viewTransform.localPosition = Vector3.Lerp(_surfer.moveData.viewTransformDefaultLocalPos,
                    _surfer.moveData.viewTransformDefaultLocalPos * crouchingHeight + Vector3.down * heightDifference * 0.5f, crouchLerp);
            else
                _surfer.moveData.viewTransform.localPosition = Vector3.Lerp(_surfer.moveData.viewTransformDefaultLocalPos - Vector3.down * heightDifference * 0.5f,
